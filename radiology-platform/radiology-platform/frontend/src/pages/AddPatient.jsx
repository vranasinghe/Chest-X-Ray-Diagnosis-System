...
export default function AddPatient() {
  ...
  function clearForm() {
    setForm({ name: '', dob: '', gender: 'Male', phone: '', email: '', address: '', notes: '', consent: false });
    setError('');
  }
  ...
  async function handleSubmit() {
    ...
    setSubmitting(true);
    try {
      const result = await api.addPatient({
        name: form.name,
        mrn: '',
        email: form.email,
        dob: form.dob,
      ...
      if (!form.consent && form.email && result?.id) {
        try { await api.sendOtp(result.id); } catch (_) {}
        navigate(`/patients/${result.id}/verify`, { state: { email: form.email } });
      } else {
        navigate('/patients');
      }
    } catch (err) {
    ...
    <Layout>
      <style>{`
        ...
        .reg-consent-label {
          font-size: 13px;
          font-weight: 700;
          color: ${`var(--consent-color, #ef4444)`};
        }
      ...
      <div className="reg-page">
        <div className="reg-container">
          ...
          {/* Back button */}
          <button className="reg-back-btn" onClick={() => navigate('/patients')}>
          ...
          {/* Header */}
          <h1 className="reg-title">New Patient Registration</h1>
          <p className="reg-subtitle">Add a patient to the roster and set their consent status.</p>
          ...
          <div className="reg-actions">
            <button className="btn-reg-clear" onClick={clearForm}>Clear Form</button>
